...
function normalizeRangeFold(result) {
  ...
  if (needsRange || needsFold) {
    chrome.storage.local.set({ range, fold });
  }
  return { range, fold };
}
...
function createRangeTable() {
  ...
  chrome.storage.local.get(["fold", "range"], (result) => {
      ...
      for (let i = 0; i < 13; i++) {
          for (let j = 0; j < 13; j++) {
              let index = (i * 13) + j;
              let name = range[index].replaceAll("10", "T").split(" ").join("");
              ...
              var box = createBox(name, index, fold[index]);
              rangeTable.append(box);
          }
          rangeTable.append('<br>');
      }

      // jQuery selectable (initialize once)
